...
def main():
    ...
    dataset = 'FSC147'
    infer_time = 0
    countzes(args, dataset, images_path, masks_path, output_path, sam, grounding_dino_model, counter_model, infer_time, clip_model, clip_processor, anyup)



def countzes(args, obj_name, images_path, masks_path, output_path, sam, grounding_dino_model, counter_model, infer_time, clip_model, clip_processor, anyup):
    ...
    json_file_path = './data/FSC-147/test.json'
    image_class_file_path = './data/FSC-147/ImageClasses_FSC147.txt'
    data_list = []
    count_item = 0
    mae = 0
    mse = 0
    ...
    with open(json_file_path, 'r') as f:
        for line in f:
            try:
                ...
                continue
    preprocessed_json_file_path = './data/FSC-147/preprocessed_test.json'
    with open(preprocessed_json_file_path, 'w') as f:
        json.dump(data_list, f, indent=4)
    print(f"Preprocessed JSON data saved to {preprocessed_json_file_path}")
    json_file_path = './data/FSC-147/preprocessed_test.json'
    json_fsc147d = './data/FSC-147/FSC-147-D.json'
    image_folder_path = './data/FSC-147/images_384_VarV2'
    try:
        with open(json_file_path, 'r') as f:
            data = json.load(f)
    except json.JSONDecodeError as e:
    ...
    for entry in data:      
        ...
        test_image_path = image_folder_path + '/' + filename
        test_image = cv2.imread(test_image_path)
        test_image = cv2.cvtColor(test_image, cv2.COLOR_BGR2RGB)


        class1 = [get_class_name(filename, image_class_file_path)]
        with open(json_fsc147d, 'r') as f:
            json_data_fsc147d = json.load(f)
        ...
        if args.visualize == True:
            set_path = './outputs/' + 'countzes_fsc147'
            if not os.path.exists(set_path):
                os.mkdir('./outputs/countzes_fsc147')
